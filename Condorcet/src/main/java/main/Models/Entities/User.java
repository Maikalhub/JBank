...
public class User {
    ...
    private Client client2;  // Ссылка на клиент
    ...
    public void admin_get_login_and_password2(String username2, String password2) throws SQLException {
        this.username2 = username2;
        this.password2 = password2;

        Client client2 = new Client();

        client2.setLogin(username2);
        client2.setPassword(password2);

        System.out.println("------------");
        System.out.println(client2.getLogin());
        ...
        mydatapane1_login.setText(username2);
        mydatapane1_password.setText(password2);
        System.out.println("Конец");
        System.out.println("Данные переданы для добавления в бд");
        JDBC.connect();
        update_system_all_data2(username2, password2);
        JDBC.close();
        //
    ...
    private void update_system_all_data2(String login, String password) throws SQLException {
        String query = "SELECT * FROM UserID WHERE Логин = ?";
        try (PreparedStatement stmt = JDBC.connection.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery())
            {
                if (rs.next())
                {
                    ...
                    update_groups(FullName, Numergroups);
                    update_works(FullName, Numergroups);
                    update_archive(FullName, Numergroups);
    ...
    public void update_groups(String login, String numbergroups) throws SQLException
    {
        // Создаем объект Client с логином и паролем

        Client client = new Client(login, numbergroups);
        ...
        requestModel.setRequestType(RequestType.SELECT_GROUP_USER); // Устанавливаем тип запроса как SELECT_GROUP
    ...
    public void update_works(String login, String password) throws SQLException {
        ...
        // Логируем логин и пароль
        System.out.println("ФИО: " + client.getLogin());
        System.out.println("Номер Группы: " + client.getPassword());

        // Создаем запрос и сериализуем клиента в JSON
        Request requestModel = new Request();
        requestModel.setRequestMessage(new Gson().toJson(client)); // Сериализация клиента
        requestModel.setRequestType(RequestType.SELECT_WORKS_USER); // Устанавливаем тип запроса как SELECT_GROUP
    ...
    public void update_archive(String login, String password) throws SQLException {
        ...
        Request requestModel = new Request();
        requestModel.setRequestMessage(new Gson().toJson(client)); // Сериализация клиента
        requestModel.setRequestType(RequestType.SELECT_ARCHIVE_USER); // Устанавливаем тип запроса как SELECT_GROUP
    ...
    public void update(ActionEvent actionEvent)
    {
        ...
        //
        try {
            admin_get_login_and_password2(username2, password2);
        } catch (SQLException e) {
    ...
    public void update_from_authorization2(String username2, String password2) {
        this.username2 = username2;
        this.password2 = password2;
    ...
    void mydatapane2_confirm_function(ActionEvent event)
    {
        ...
        else
        {
            ...
            double salary_mydata = Double.valueOf(mydatapane2_salary.getText());
            ...
            mydatapane2_confirm_function_edit_AdminID(fio_mydata,login_mydata,password_mydata,numbergroups_mydata,post_mydata,status_mydata,salary_mydata);
    ...
    void mydatapane2_confirm_function_edit_AdminID
        ...
        UserID newUser;
        newUser = new UserID(0,login_datagroups_, password_datagroups_, fio_datagroups_, post_datagroups_, sdstatus_datagroups_,salary_datagroups_, "BYR",  numbergroups_datagroups_);
    ...
    @FXML
    void reportpane_add_function(ActionEvent event) {}
    @FXML
